      ...
      if (b) {
        ...
        if (!this.book.authors) {
          ...
          this.book.tags = this.book.tags.reduce((tags, x) => {
            const tag = this.$store.state.sortedTags.reduce(
              (acc, t) => (t.tag === x ? t : acc),
              null,
            )
            if (tag) {
  ...
  <section class="section">
    <form class="w-100" @submit.prevent="save">
        ...
        <div class="column is-one-third">
          ...
          <div>
            <button
              ...
              :disabled="$uiBusy || searching"
              :class="{ 'is-loading': searching }"
              @click.prevent="reloadCover"
            >
              Reload
            </button>
          </div>
        ...
        <div class="column is-two-thirds">
          ...
          <div class="field">
            ...
            <button
              ...
              class="button is-secondary is-small d-inline ml-5"
              @click.prevent="delAuthor"
            >
            ...
            <div v-for="(person, index) of book.authors" :key="index" class="mt-2 field is-grouped">
              <field-person
                :person="book.authors[index]"
                @changed="updatePerson($event, index)"
                @changed-role="updatePersonRole($event, index)"
              />
            </div>
          </div>
          ...
          <div class="field">
            ...
            <div v-for="tag of $store.state.sortedTags" :key="tag.id" class="control">
              ...
              <label class="label d-inline" :for="tag.id">
                {{ tag.tag }}
                <i v-if="tag.showOnFront" class="fas fa-check has-text-primary ml-1" />
              </label>
            </div>
          </div>
      ...
      <div>
        ...
        <router-link class="button is-secondary ml-2" :to="{ name: 'BooksManager' }"
          >Cancel</router-link
        >
      </div>
    </form>
