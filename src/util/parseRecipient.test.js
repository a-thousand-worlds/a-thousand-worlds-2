...
test('first last email@domain.com', () => {
  expect(parseRecipient('Luke Skywalker luke@rebelalliance.org'))
    .toEqual({
      firstName: 'Luke',
      lastName: 'Skywalker',
...
test('no false positive on dashed names', () => {
  ...
  expect(parseRecipient('Abra Ca-Dabra - abra@cadabra.org'))
    .toEqual({
      firstName: 'Abra',
      lastName: 'Ca-Dabra',
...
test('first name and email only', () => {
  expect(parseRecipient('Luke luke@rebelalliance.org'))
    .toEqual({
      firstName: 'Luke',
      lastName: '',
      email: 'luke@rebelalliance.org',
...
test('email only', () => {
  expect(parseRecipient('luke@rebelalliance.org'))
    .toEqual({
      firstName: '',
      lastName: '',
      email: 'luke@rebelalliance.org',
...
test('return invalid result if no email', () => {
  expect(parseRecipient('Luke Skywalker'))
    .toEqual({
      isValid: false,
      raw: 'Luke Skywalker',
    })
