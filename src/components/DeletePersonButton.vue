...
export default {
  ...
  computed: {
    books() {
      return this.$store.getters['books/list']().filter(
        book =>
...
<template>
  <button
    ...
    v-tippy="{
      content:
        books.length > 0
          ? `${person.name} has ${books.length} book${
              books.length === 1 ? '' : 's'
            } and may not be deleted. If all of their books are deleted, then they can be deleted.`
          : `${person.name} has no books and may be deleted`,
    }"
